...
static void test_to_x(void) {
    ...
    for (size_t i = 0; i < sizeof(to_tests) / sizeof(to_tests[0]); i++) {
        ...
        // printf("%lld\n", d);
        assert(duration_to_micro(d) == test.golden.usec);
        assert(duration_to_milli(d) == test.golden.msec);
        assert(round(duration_to_seconds(d)) == round(test.golden.sec));
        assert(round(duration_to_minutes(d)) == round(test.golden.min));
        assert(round(duration_to_hours(d)) == round(test.golden.hour));
    }
    printf("OK\n");
}
...
static void test_to_minutes(void) {
    ...
    for (size_t i = 0; i < sizeof(to_minute_tests) / sizeof(to_minute_tests[0]); i++) {
        ...
        double got = duration_to_minutes(test.d);
        // printf("want %f, got %f\n", test.want, got);
...
static void test_to_hours(void) {
    ...
    for (size_t i = 0; i < sizeof(to_hour_tests) / sizeof(to_hour_tests[0]); i++) {
        ...
        double got = duration_to_hours(test.d);
        // printf("want %f, got %f\n", test.want, got);
        assert(got == test.want);
    }
    printf("OK\n");
...
static void test_truncate(void) {
    ...
    for (size_t i = 0; i < sizeof(truncate_tests) / sizeof(truncate_tests[0]); i++) {
        ...
        Duration got = duration_truncate(test.d, test.m);
...
static void test_round(void) {
    ...
    for (size_t i = 0; i < sizeof(round_tests) / sizeof(round_tests[0]); i++) {
        ...
        Duration got = duration_round(test.d, test.m);
        // printf("want %lld, got %lld\n", test.want, got);
        assert(got == test.want);
    }
    printf("OK\n");
...
static AbsTest abs_tests[] = {
    ...
    {DURATION_MIN + 1, DURATION_MAX},
    {DURATION_MIN + 2, DURATION_MAX - 1},
    {DURATION_MAX, DURATION_MAX},
    {DURATION_MAX - 1, DURATION_MAX - 1},
};
...
static void test_abs(void) {
    ...
    for (size_t i = 0; i < sizeof(abs_tests) / sizeof(abs_tests[0]); i++) {
        ...
        Duration got = duration_abs(test.d);
        // printf("want %lld, got %lld\n", test.want, got);
        assert(got == test.want);
    }
    printf("OK\n");
