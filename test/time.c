...
static void test_tm(void) {
    ...
    for (size_t i = 0; i < sizeof(calendar_tests) / sizeof(calendar_tests[0]); i++) {
        ...
        // test.tm_mon, test.tm_mday, test.tm_hour, test.tm_min, test.tm_sec,
        // test.offset_sec / 3600, time_to_unix(want), time_to_unix(got));
...
static AddTest add_tests[] = {
    ...
    // 2019-08-16 02:29:30.268436582 + 9223372036795099414 = 2311-11-26
    // 02:16:47.63535996
...
static void test_add_to_exact_second(void) {
    ...
    // This test checks that the nsec field still lies within the range [0,
    // 999999999].
...
static SubTest sub_tests[] = {
    ...
    {{-9223372036, -854775808}, {4414590000, 0}, DURATION_MIN},
    // 2109-11-23 - max = 0001-01-01
    {{4414590000, 0}, {-9223372036, -854775808}, DURATION_MAX},
    // 0001-01-01 - max = -2109-11-23
    {{-9223372036, -854775808}, {-128692627200, 0}, DURATION_MAX},
    // 2290-01-01 - (290*365*24h + 71*24h) = 2000-01-01
    {{10098259200, 0}, {946684800, 0}, (290 * 365 + 71) * (24 * 3600e9)},
    // 2300-01-01 - max = 2000-01-01
    {{10413792000, 0}, {946684800, 0}, DURATION_MAX},
    // 2000-01-01 + 290*365*24h + 71*24h = 2290-01-01
    {{946684800, 0}, {10098259200, 0}, (-290 * 365 - 71) * (24 * 3600e9)},
    // 2000-01-01 - min = 2300-01-01
    {{946684800, 0}, {10413792000, 0}, DURATION_MIN},
    // 2311-11-26 02:16:47.63535996 - 9223372036795099414 = 2019-08-16
    // 02:29:30.268436582
...
static void test_marshal_binary(void) {
    printf("test_marshal_binary...");
    uint8_t buf[15];
    for (size_t i = 0; i < sizeof(unix_tests) / sizeof(unix_tests[0]); i++) {
        TimeTest test = unix_tests[i];
        Time want = time_unix(test.sec, test.nsec);
        time_marshal_binary(want, buf);
        Time got = time_unmarshal_binary(buf);
        // printf("want: {%lld %lld}, got: {%lld %lld}\n", want.sec, want.nsec,
        // got.sec, got.nsec);
        assert(time_equal(got, want));
    }
